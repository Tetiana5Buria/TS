...
function getAllProductNames(
  a: { products?: { name?: string }[] } | undefined
): string[] {
  //optional
  return a?.products?.map((prod) => prod?.name || "") || [];
}
...

{
  ...
  console.log(
    " function 4.1a:",
    hey({ name: () => "Roman", cuteness: 100 } as {
      name: () => string;
      cuteness: number;
    })
  );
  ...
  console.log(
    " function 4.1b:",
    hey({ name: () => "Vasyl", coolness: 100 } as {
      name: () => string;
      coolness: number;
    })
  );
...

{
  ...
  function heyHard(a: Men | Person) {
    return "hey! i'm " + a.name();
  }
  console.log(
    " function 4.1 UnionType:",
    heyHard({ name: () => "roman", cuteness: 100 })
  );
  console.log(
    " function 4.1 UnionType:",
    heyHard({ name: () => "vasyl", coolness: 100 })
  );
}
...

{
  class AbstractPet<T> {
    ...
    constructor(name: () => string, property: T) {
      this.name = name;
      this.property = property;
    }
  }

  class Cat extends AbstractPet<boolean> {}
  class Dog extends AbstractPet<number> {}

  function hey(abstractPet: AbstractPet<any>): string {
    return "hey! i'm " + abstractPet.name();
  }
...
{
  type Pets =
    | { name: () => string; type: "cat"; cuteness?: number }
    | { name: () => string; type: "dog"; coolness?: number };

  /* or second option
     type Cat = { name: () => string; type: "cat"; cuteness: number };
type Dog = { name: () => string; type: "dog"; coolness: number };
type Pets = Cat | Dog; */
  function hey(a: Pets): string {
    return (
      "hey! i'm " +
      a.name() +
      ", " +
...
const hello: () => Promise<string> = () => { // resolve
